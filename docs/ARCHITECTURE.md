...
FMPClient / SECClient
  ↓
Financial Modeling Prep API / SEC EDGAR API
```
...
### SEC earnings release fetch

`get_earnings_release_json` is the canonical source for official SEC EDGAR earnings-release documents. It accepts the selected quarter context:
...
The SEC client resolves the ticker to a CIK, reads the company submissions API, selects a likely 8-K/6-K earnings-release exhibit, fetches the SEC document, and converts HTML/TXT into LLM-friendly JSON. The payload includes source URLs, selected filing metadata, text blocks, and tables when requested. Raw HTML is omitted by default.
...
- required SEC earnings-release fetch actions for every selected period
...
- official SEC earnings releases must be fetched and read for every selected period
