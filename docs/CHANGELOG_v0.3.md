...
### SEC EDGAR earnings-release JSON

1. Add `get_earnings_release_json(symbol, fiscalYear, fiscalQuarter, filingDate, includeHtml=false, includeTables=true)`.
2. Add a read-only SEC EDGAR client that resolves ticker to CIK, fetches submissions metadata, selects a likely 8-K/6-K earnings-release exhibit, and fetches the selected SEC document.
3. Convert SEC HTML/TXT documents into LLM-friendly JSON with `release_json.text`, `release_json.blocks`, and optional `release_json.tables`.
4. Keep raw HTML out of the response by default; return it only when `includeHtml=true`.
5. Add mandatory SEC earnings-release read actions to `fmp_build_research_evidence_pack` for every selected period.
6. Update strict workflow blockers so scoring requires the agent to fetch and read the official earnings release for each selected period.
7. Bump package, contract, and evidence-pack versions to `0.3.3`.

## Version 0.3.2

### Read-only MCP tool metadata

1. Add explicit MCP `ToolAnnotations` to every exposed action.
2. Add human-readable titles and “Use this when...” descriptions.
3. Add stricter input validation for ticker symbols and ISO date ranges.
4. Add a contract test that fails if a tool is missing read-only annotations.
